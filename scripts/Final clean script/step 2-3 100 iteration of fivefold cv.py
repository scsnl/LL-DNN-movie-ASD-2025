"""
Step 2-3: Repeated Cross-Validation Stability Analysis

This script conducts 100 independent runs of 5-fold cross-validation to assess 
the stability and generalizability of the DualPathNet architecture. 
For each iteration:
1. Re-shuffles the dataset and generates a new set of stratified 5-fold splits.
2. Trains the model from scratch on each fold.
3. Records validation accuracy and F1-score.
4. Aggregates results to visualize the distribution of performance across all runs.
"""
...
# Optional Pytorch and Lightning imports
try:
    import torch
    ...
    torch = nn = DataLoader = TensorDataset = pl = EarlyStopping = None
...
# Configuration for test mode and data paths
...
# Skip repeated CV in test mode to keep smoke tests fast
if TEST_MODE:
    print("[TEST_MODE] Skipping 100-run CV stability analysis.")
    raise SystemExit(0)
...
# Architecture definition (consistent with main training script)
...
class DualPathPL(pl.LightningModule):
    ...
    def forward(self, x, meta):
        self.model.to(x.device)  # Force device synchronization
...
def main():
    ...
    for run in range(n_runs):
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(fmri, labels)):
            print(f"[Run {run+1}/{n_runs} | Fold {fold+1}/5]")
            ...
            for xb, mb, yb in val_loader:
                ...
                pred = torch.argmax(fusion_logits, dim=1)
                preds.extend(pred.cpu().numpy()); targets.extend(yb.cpu().numpy())
    ...
    out_dir = os.path.join(base_out, "crossval_results")
    os.makedirs(out_dir, exist_ok=True)
    with open(os.path.join(out_dir, "dualpathnet_stability_results.json"), "w") as f:
    ...
    plt.ylim(65, 80)
    plt.ylabel("Accuracy (%)")
    plt.title(f"Accuracy Distribution (DualPathNet: {n_runs}x5-Fold)")
    plt.text(0, mean_acc, f"Mean: {mean_acc:.2f}%\nStd: {std_acc:.2f}%", ha='center', va='center', fontsize=10,
             bbox=dict(facecolor='white', edgecolor='gray'))
    plt.grid(True, axis='y', linestyle='--', alpha=0.5)
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "cv_stability_boxplot.png"))
    plt.close()
