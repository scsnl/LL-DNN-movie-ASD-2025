...
mkdir -p "$OUT_DIR/saved_models" "$OUT_DIR/cv_indices" "$OUT_DIR/crossval_results" "$OUT_DIR/nested_results"
...
ln -sfn "$OUT_DIR/nested_results" "nested_results"

# Step 1: CV indices
python "step 1 generate_cv_indices.py"

# Step 2: Train + IG (TEST_MODE=1: epochs=1)
python "step 2-1 5-fold training and ig.py"

# Step 2: Nested CV (TEST_MODE=1: N_HP_CONFIGS=2 for minimal run)
python "step 2-1 nested cv runner.py"

# Step 2: Metrics summary (reads nested_cv_summary.json)
